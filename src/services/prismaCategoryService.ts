...
export class PrismaCategoryService {
  ...
  static async create(data: CreateCategoryRequest): Promise<Category> {
    this.logDatabaseOperation('INSERT INTO categories (account_id, name, type, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())', `${data.name} (${data.type}) for account ${data.accountId}`);
